...
export async function POST(request:Request,{params}:{params:{companyId:string}}){
  try{
    ...
    return NextResponse.json({ok:true,document:{id:result.document.id,originalFileName:result.document.originalFileName,fileType:result.document.fileType,mimeType:result.document.mimeType,fileSize:result.document.fileSize.toString(),documentStatus:result.document.documentStatus,uploadedBy:result.document.uploadedBy.name,createdAt:result.document.createdAt.toISOString()}});
