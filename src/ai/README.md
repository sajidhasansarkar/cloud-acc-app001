...
- `gemini`, selected via `ACCOUNTING_AI_PROVIDER=gemini` and `GEMINI_API_KEY`, calls Google's Gemini API using structured JSON output. It only ever selects an account and explains/flags its reasoning — debit/credit/amount are always taken verbatim from the normalized transaction candidate, never generated by the model, so a hallucinated number can't reach persistence. Any account id outside the supplied `relevantAccounts` context is rejected client-side; `review.ts` independently re-validates every account and value again regardless of provider.
