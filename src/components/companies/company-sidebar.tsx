...
export function CompanySidebar({
  ...
  // Hovering only "peeks" the sidebar open while it's actually collapsed —
  // it never does anything when the sidebar is already expanded, and it
  // never changes (or persists) the real collapsed preference. Moving the
  // mouse away collapses it back to icons.
  const [isHovered, setIsHovered] = useState(false);
  const isPeeking = isCollapsed && isHovered;
  const expanded = !isCollapsed || isPeeking;

  return (
    // Outer box reserves real layout space (icon-rail width when collapsed)
    // so the rest of the page never reflows on hover. The actual sidebar is
    // an absolutely positioned panel inside it that's free to widen over the
    // page content for the peek effect.
    <div
      className={cn(
    ...
    >
      <aside
        className={cn(
          "absolute inset-y-0 left-0 z-20 flex flex-col border-r border-ink-800 bg-ink-950 text-ink-300",
          hydrated && "transition-[width] duration-200 ease-in-out",
          expanded ? "w-64" : "w-16",
          isPeeking && "shadow-xl shadow-black/30"
        )}
      >
        {/* Toggle handle — sits on the sidebar's right border so it stays put
            whether the sidebar is expanded or collapsed. Always reflects (and
            controls) the persisted preference, not the hover peek. */}
        <button
          type="button"
          onClick={toggle}
        ...
        <div
          className={cn(
            ...
            expanded ? "gap-2 px-5" : "justify-center px-0"
          )}
        >
          ...
          <ul className="space-y-0.5">
            {COMPANY_NAV_ITEMS.map((item) => {
              if (item.label === "Reports") {
                return (
                  <li key={item.label}>
                    <ReportsDropdown companyId={companyId} collapsed={!expanded} />
                  </li>
                );
              }
                ...
                return (
                  <li key={item.label}>
                    <span
                      ...
                      title={!expanded ? `${item.label} — coming in a future phase` : undefined}
                    >
                      <span className={cn("flex items-center", expanded && "gap-2.5")}>
        ...
        <div className="border-t border-ink-800 p-3">
          <Link
            ...
            title={!expanded ? "All companies" : undefined}
            className={cn(
