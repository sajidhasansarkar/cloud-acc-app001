...
export function GeneralLedgerFilterBar({
  ...
  values: { accountId?: string; accountSearch?: string; dateFrom?: string; dateTo?: string; fiscalYearId?: string; accountingPeriodId?: string };
    ...
    <form method="get" className="rounded-lg border border-ink-100 bg-white p-4 shadow-card">
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        ...
        <label className="space-y-1.5">
          ...
          <Select name="accountId" defaultValue={values.accountId ?? ""} disabled={accountLocked}>
      ...
      {accountLocked ? <input type="hidden" name="accountId" value={values.accountId ?? ""} /> : null}
